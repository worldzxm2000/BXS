...
{
	QString name = QString::fromLocal8Bit("便携式气象站业务");
	return name;
}
		...
		if (buff[i] == 0xaa)
		{
			...
			for (int j = i + 1; j < buff.length(); j++)
			{
				if (buff[j] == 0xdd)
				{
					...
					QString fileName = QCoreApplication::applicationDirPath() + "\\BXS\\" + QString::number(frame.SrcAddr) + "\\" + current_day;
					QDir dir(fileName);
					if (!dir.exists())
...
void Transform2SessiorStatus(QString data, QJsonObject &json)
{
	...
	value.sprintf("%d%d%d%d%d%d%d%d%d%d%d",
		...
		(int)(data.at(9).toLatin1()), (int)(data.at(10).toLatin1()));
